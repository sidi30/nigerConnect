...
describe('FriendsService', () => {
  it('cannot send request to self', async () => {
    ...
    const svc = new FriendsService(prisma, makeBlocks() as never);
  ...
  it('blocks friend request when users are blocked', async () => {
    const prisma = { friendship: {}, user: {} } as never;
    const svc = new FriendsService(prisma, makeBlocks(true) as never);
  ...
  it('throws NotFound when addressee does not exist', async () => {
    ...
    const svc = new FriendsService(prisma as never, makeBlocks() as never);
  ...
  it('accept marks friendship as accepted', async () => {
    const prisma = {
      friendship: {
        findUnique: jest.fn(async () => ({ id: 'f1', addresseeId: 'me', status: 'pending' })),
